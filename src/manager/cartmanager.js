...
class CartManager {
    ...
    #newID; // Variable privada para gestionar los ID de los productos
    ...
    async addCart() {
        try { 
            const fileCarts = await this.getCarts(); // Se obtiene los productos del archivo
...
async addProductToCart(cartId, productId){
    try {
        ...
        if (cart) {
            ...
            if (productIndex !== -1) { // Si el producto  existe en el carrito, incrementa la cantidad en 1
                fileCarts[cartIndex].products[productIndex].quantity += 1;

            } else { // Si el producto no existe en el carrito, se agrega con una cantidad inicial de 1
